...
async function signupUser({ name, email, password, tenantId, isSuperadmin }) {
  ...
  try {
    return await prisma.$transaction(async (tx) => {
      ...
      // Send verification email outside the transaction to avoid blocking
      setImmediate(() => {
        sendVerificationEmail(email, verificationToken, isSuperadmin).catch(error => {
      ...
      return { 
        ...
        tenantId: tenantId ? tenantId : tenant.id, 
        email, 
        assignedRoleId: defaultRoleId 
      };
    }, {
      timeout: 10000, // 10 second timeout
    });
  } catch (error) {
    console.error('Transaction error in signupUser:', error);
    throw error;
  }
}
